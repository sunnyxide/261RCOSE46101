...
Table~\ref{tab:crosscultural} reports mean KS distance per (adapter, target culture). In-distribution cells (bold) show the lowest KS for their target culture, confirming Cultural-QLoRA's distributional shift toward the trained culture. The transfer matrix (off-diagonal cells) reveals that cultural conditioning reweights rather than deletes other-culture capability: Cultural-KR's KS to JP/US/CN remains within 0.05 of Vanilla-3B baseline. Table~\ref{tab:blend} reports companion BLEnD MCQ accuracy.
...
\midrule
Vanilla-3B & 0.590 & 0.593 & - & - \\
Vanilla-7B & 0.607 & - & - & - \\
Cultural-KR (3B) & \textbf{0.597} & - & - & - \\
Cultural-JP (3B) & 0.615 & \textbf{0.621} & - & - \\
Cultural-US (3B) & 0.705 & - & - & - \\
Cultural-CN (3B) & 0.629 & 0.639 & - & - \\
Cultural-KR (7B) & \textbf{0.535} & - & - & - \\
Multi-cultural & 0.619 & - & - & - \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Vanilla-3B & 0.388 & 0.000 & - & - \\
Vanilla-7B & 0.388 & - & - & - \\
Cultural-KR (3B) & 0.388 & - & - & - \\
Cultural-JP (3B) & 0.362 & 0.000 & - & - \\
Cultural-US (3B) & 0.087 & - & - & - \\
Cultural-CN (3B) & 0.388 & 0.000 & - & - \\
Cultural-KR (7B) & 0.388 & - & - & - \\
Multi-cultural & 0.388 & - & - & - \\
\bottomrule
\end{tabular}
\end{table}=0.590$ (mean), $0.600$ (median). Final 8 models $\times$ 4 cultures = 32-cell matrix will be aggregated when AWS evaluation completes; results inserted via \texttt{scripts/aggregate\_results.py}.}

\paragraph{Hofstede dimension ablation.}
Table~\ref{tab:hofstede} compares Hofstede-conditioning variants on Korean WVS alignment. Single-dimension conditioning (IDV-only or UAI-only) captures a substantial fraction of full-6D's shift, supporting the soft-prior interpretation.
...
\midrule
Full Cultural-QLoRA (Hofstede + cultural data) & 0.597 \\
\bottomrule
\end{tabular}
\end{table}
